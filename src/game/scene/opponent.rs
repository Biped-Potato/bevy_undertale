...
impl Plugin for OpponentPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_systems(OnEnter(AppState::Level), spawn_opponent)
            .add_systems(
            Update,
...
fn update_opponent_position(
    ...
    for(mut opponent,mut physics) in opponent_query.iter_mut() {
        ...
        physics.position.y = bullet_board.position.y + bullet_board.height / 2.0 + bullet_board.border + 10.0 + data.game.opponent_data.height / 2.0;
    }
}
