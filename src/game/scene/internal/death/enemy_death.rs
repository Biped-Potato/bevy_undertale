...
use crate::game::{data::data::Data, physics::physics_object::PhysicsComponent, player::player::Player, scene::internal::{helpers::despawn::DespawnInTime, menu::MenuState, opponent::Opponent}};
...
impl Plugin for EnemyDeathPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(FixedUpdate,activate_dust.run_if(in_state(MenuState::EnemyDeath)));
    }
}
    ...
    if d_a.timer <= 0. && d_a.i < d_a.rows.len() as i32 {
        ...
        for i in 0..l {
            if let Ok(mut p) = query.get_mut(row[i]) {
                ...
                commands.entity(row[i]).insert(DespawnInTime{time : data.game.opponent_data.dust_life,effect : None});
            }
        }
        d_a.i+=1;
    }
