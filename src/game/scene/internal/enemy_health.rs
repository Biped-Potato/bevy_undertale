...
use crate::game::{data::data::Data, loading::loading::AssetManager, scene::internal::{bullet_board::BulletBoard, decisions::Decisions, progress::Progress, stats::{HealthBar, HealthBarType}}, state::state::AppState};

pub struct EnemyHealthPlugin;
impl Plugin for EnemyHealthPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_systems(FixedPreUpdate, make_enemy_bar_invis)
...
pub fn manage_enemy_healthbar(
    ...
    mut enemy_healthbar : Query<(&mut HealthBar,&mut HealthBarType, &mut EnemyHealthBar,&mut Visibility)>,
...
fn spawn_bar(
    ...
    let pos = Vec2::new(
        data.game.player.sprite_size_x / 2.0 + b_board.position.x
            - b_board.width / 2.0
            + 27.0,
            -data.game.player.sprite_size_y / 2.0 + b_board.position.y + b_board.height / 2.0
            - 23.0
