...
impl Plugin for OpponentPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(OnEnter(AppState::Level), (spawn_opponent,spawn_opponent_health_ui))
            .add_systems(
                FixedUpdate,
                update_opponent_position.before(update_enemy_healthbar).run_if(in_state(AppState::Level)),
            )
    ...
    for (mut opponent, mut physics) in opponent_query.iter_mut() {
        ...
        if fight_manager.strike {
            ...
            if fight_manager.attack_animation <= 1.0 {
                ...
                opponent.offset = Vec2::X * f32::sin(time * 2.0 * 3.14159 * shake_speed) * 10.0 * fight_manager.attack_animation;
...
fn update_enemy_healthbar(
    ...
    mut text_query : Query<(&mut DamageText,&mut Visibility, &mut Transform,&mut TextLayoutInfo,Entity),Without<OpponentHealthBar>>,
        ...
        for(mut h_t,mut h,mut b,mut v,mut t) in bar_query.iter_mut() {
            if fight_manager.strike {
                if fight_manager.attack_animation <= 1.0 {
                    ...
                    h.position = physics.position - o.offset + Vec2::new(0.,data.game.opponent_data.height / 2.0) + Vec2::new(0.,h.custom_size.unwrap().y as f32 / 2.0);
            ...
            if fight_manager.strike {
                if fight_manager.attack_animation <= 1.0 {
                    *writer.text(e, 0) = bar_manager.damage_display.to_string();
                    let mut position = healthbar_pos + Vec2::new(0., healthbar_height);
                    let time = f32::clamp((1.0 - fight_manager.attack_animation) * 2.0,0.0,1.0);
