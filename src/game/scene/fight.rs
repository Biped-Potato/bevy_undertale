...
use crate::game::{data::data::Data, loading::loading::AssetManager, player::player::Player, scene::{bullet_board::{self, BulletBoard}, decisions::Decisions, menu::MenuState, menu_transition::MenuTransition}, state::state::AppState};

pub struct FightPlugin;
impl Plugin for FightPlugin {
    fn build(&self, app: &mut App) {
        app
            .init_resource::<FightManager>()
            .add_systems(OnEnter(AppState::Level),spawn_fight_bar)
            .add_systems(OnEnter(MenuState::Fight),init_fight)
            .add_systems(FixedUpdate,update_visibility.run_if(in_state(AppState::Level)))
            .add_systems(Update,update_fight_controls.run_if(in_state(MenuState::Fight)))
            .add_systems(FixedUpdate,update_fight_bar.run_if(in_state(MenuState::Fight)))
            .add_systems(Update,update_player_visibility.run_if(in_state(MenuState::Fight)));
    }
}
